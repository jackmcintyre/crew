...
export async function removeInProgressSnapshot(opts) {
    ...
    try {
        ...
        const code = err?.code;
        if (code === "ENOENT")
            return; // best-effort — already gone
        throw err;
    }
}
/**
...
export async function detectInProgressHandEdit(opts) {
    ...
    if (!operatorFieldsEqual(diskFields, snapshotFields)) {
        if (diskFields.title !== snapshotFields.title)
            changedFields.push("title");
        if (diskFields.narrative !== snapshotFields.narrative)
        ...
        if (diskFields.withdrawn !== snapshotFields.withdrawn)
