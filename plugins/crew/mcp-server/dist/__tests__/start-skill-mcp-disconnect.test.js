...
const HALT_LINE = `[mcp-cascade-halted] MCP child killed by subagent Task termination — ` +
 ...
 */
async function wrapMcpCall(methodName, ref, call) {
    try {
        return await call();
    }
    catch (err) {
...
describe("AC4(d) — wrapper halts inner cycle on disconnect", () => {
    it("re-raises as McpDisconnectedError; no further MCP calls attempted", async () => {
        ...
        let typedError = null;
        try {
            await wrapMcpCall("processDevTranscript", claimed.ref, processDevTranscript);
            // Only reached if the wrapper failed to throw — which would be a bug.
            await wrapMcpCall("processReviewerTranscript", claimed.ref, processReviewerTranscript);
        }
        catch (err) {
            typedError = err;
    ...
    it("non-disconnect errors propagate unchanged through the wrapper", async () => {
        ...
        await expect(wrapMcpCall("foo", "ref", boom)).rejects.toThrow("validation failed");
        await expect(wrapMcpCall("foo", "ref", boom)).rejects.not.toBeInstanceOf(McpDisconnectedError);
    });
});
