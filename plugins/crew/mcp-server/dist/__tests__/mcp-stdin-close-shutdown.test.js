...
describe("AC4e — tool dispatch works through the socket transport", () => {
    it("responds to tools/list after init handshake over the unix socket", async () => {
...
describe("D2 contract — daemon survives client socket disconnect", () => {
    it("daemon remains alive after the client socket is destroyed", async () => {
        harness = await spawnDaemonHarness({ distIndex: DIST_INDEX });
        await harness.initHandshake();
        let exited = false;
...
describe("AC4c — SIGTERM terminates the daemon cleanly", () => {
    it("daemon exits within 5 seconds of SIGTERM", async () => {
        harness = await spawnDaemonHarness({ distIndex: DIST_INDEX });
        await harness.initHandshake();
        ...
        const { code, signal } = await Promise.race([
            exitPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error("SIGTERM did not kill daemon within 5 seconds")), 5_000)),
        ]);
