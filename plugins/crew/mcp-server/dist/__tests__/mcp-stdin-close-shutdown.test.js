...
describe("AC4e — tool dispatch is unaffected by the lifecycle logging setup", () => {
    it("responds to tools/list before any stdin close", async () => {
...
describe("AC4d — child does not exit during healthy steady-state", () => {
    it("child remains alive for 5 seconds under normal operation", async () => {
        await doInitHandshake(child);
        let exited = false;
        child.on("exit", () => {
            exited = true;
...
describe("AC5 — child exits cleanly on stdin close (Story 5.25 new contract)", () => {
    it("child exits with code 0 within 5 seconds after stdin is closed", async () => {
        await doInitHandshake(child);
        ...
        const result = await Promise.race([
            exitPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error("Child did not exit within 5 seconds after stdin close")), 5_000)),
        ]);
...
describe("AC4c — SIGTERM terminates the child", () => {
    it("child exits within 5 seconds of SIGTERM", async () => {
        await doInitHandshake(child);
        const exitPromise = new Promise((resolve) => {
            child.on("exit", (code, signal) => resolve({ code, signal }));
        });
        ...
        const { code, signal } = await Promise.race([
            exitPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error("SIGTERM did not kill child within 5 seconds")), 5_000)),
        ]);
