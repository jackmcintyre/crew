 ...
 *    without that file).
 */
export function makeRubberStampDevTranscript(ref) {
    return [
 ...
 */
export function composeReviewerTranscript(acResults) {
 ...
 */
export function assertVerdictTranscriptContract(reviewerTranscript) {
 ...
 */
export async function assertManifestStaysInProgress(targetRepoRoot, ref) {
    const inProgressPath = path.join(targetRepoRoot, ".crew", "state", "in-progress", `${ref}.yaml`);
    const donePath = path.join(targetRepoRoot, ".crew", "state", "done", `${ref}.yaml`);
