...
describe("AC1 / AC4a — child survives parent stdin close", () => {
    it("child does NOT exit within 10 seconds after stdin is closed", async () => {
        await doInitHandshake(child);
        let exited = false;
        child.on("exit", () => {
            exited = true;
...
describe("AC2 / AC4b — stdout is not destroyed as a side-effect of stdin close", () => {
    it("child stdout is still open (not destroyed) after stdin is closed", async () => {
        await doInitHandshake(child);
        // Close stdin
        child.stdin?.end();
        child.stdin?.destroy();
...
describe("AC3 / AC4c — SIGTERM terminates the child even after stdin close survival", () => {
    it("child exits within 5 seconds of SIGTERM after surviving stdin close", async () => {
        await doInitHandshake(child);
