...
import { UnknownAdapterError, AmbiguousAdapterError, NoAdapterMatchedError, } from "../errors.js";
import { z } from "zod";
// ---------------------------------------------------------------------------
// Helpers — two minimal stub adapters used throughout
// ---------------------------------------------------------------------------
function makeStub(name, detectResult) {
    return {
        name,
        detect: async (_repo) => detectResult,
        listSourceStories: async () => [],
        readSourceStory: async (ref) => {
            throw new Error(`stub: readSourceStory not implemented (${ref})`);
        },
        resolveSourcePath: (ref) => `/stub/${name}/${ref}`,
        validateAgainstDiscipline: (story) => story,
        defaultConfig: () => ({}),
