...
import { getAnthropicClient, hasAnthropicKey, } from "../../lib/anthropic-client.js";
/**
...
function buildUserMessage(filename, fileContents) {
    return [
        `Filename: ${filename}`,
        "",
        "File contents:",
 ...
 */
function cacheDir(targetRepoRoot) {
    return path.join(targetRepoRoot, ".crew", "state", "extraction-cache");
}
function cachePath(targetRepoRoot, sourceHash) {
    return path.join(cacheDir(targetRepoRoot), `${sourceHash}.json`);
}
async function readCache(targetRepoRoot, sourceHash) {
    try {
...
async function writeCache(targetRepoRoot, sourceHash, story) {
...
async function callModel(client, model, filename, fileContents) {
 ...
 */
export async function extractBmadStoryViaLlm(absPath, fileContents, opts) {
    ...
    try {
        ...
        if (err instanceof BmadLlmExtractionError) {
            throw new BmadLlmExtractionError({
                path: absPath,
                reason: `both primary (${primaryModel}) and retry (${retryModel}) extraction attempts failed`,
