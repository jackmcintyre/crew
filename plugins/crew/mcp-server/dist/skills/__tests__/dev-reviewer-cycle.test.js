...
function makeInMemoryDeps(devTranscripts, reviewerTranscripts) {
    const spawnCalls = [];
...
describe("(a) happy handoff + READY FOR MERGE", () => {
    it("returns finalState: 'ready-for-merge', no manifest writes, AC1 chatLog line", async () => {
        const { deps, spawnCalls, manifestWrites } = makeInMemoryDeps([HANDOFF_PHRASE], [READY_FOR_MERGE]);
...
describe("(b) NEEDS CHANGES → one rework → READY FOR MERGE", () => {
    it("writes rework_count: 1, AC2 chatLog line with <n>=1, finalState: 'ready-for-merge'", async () => {
        const { deps, spawnCalls, manifestWrites } = makeInMemoryDeps([HANDOFF_PHRASE, HANDOFF_PHRASE], // two dev spawns
        [NEEDS_CHANGES, READY_FOR_MERGE]);
        const result = await runDevReviewerCycle({
            targetRepoRoot: tmpRoot,
            sessionUlid: SESSION_ULID,
            ref: STORY_REF,
...
describe("(c) handoff grammar drift → blocked", () => {
    it("stamps blocked_by: handoff-grammar, no reviewer spawn, AC3 chatLog, finalState: blocked-handoff-grammar", async () => {
        const { deps, spawnCalls, manifestWrites } = makeInMemoryDeps(["story is ready for review — handing off!"], // paraphrase drift
        []);
        const result = await runDevReviewerCycle({
            targetRepoRoot: tmpRoot,
            sessionUlid: SESSION_ULID,
...
describe("(d) reviewer verdict drift → blocked-reviewer-grammar", () => {
    it("stamps blocked_by: reviewer-grammar, verbatim reviewer-drift line", async () => {
        const { deps, spawnCalls, manifestWrites } = makeInMemoryDeps([HANDOFF_PHRASE], ["Looks good to me!"]);
        const result = await runDevReviewerCycle({
            targetRepoRoot: tmpRoot,
            sessionUlid: SESSION_ULID,
            ref: STORY_REF,
...
describe("(e) BLOCKED verdict → blocked-reviewer-verdict", () => {
    it("no manifest write, finalState: blocked-reviewer-verdict, verbatim BLOCKED passthrough", async () => {
        const { deps, spawnCalls, manifestWrites } = makeInMemoryDeps([HANDOFF_PHRASE], [BLOCKED_VERDICT]);
        const result = await runDevReviewerCycle({
            targetRepoRoot: tmpRoot,
            sessionUlid: SESSION_ULID,
            ref: STORY_REF,
...
describe("(f) two-iteration rework: NEEDS CHANGES × 2 → READY FOR MERGE", () => {
    it("six spawns, rework_count: 2, AC2 line appears twice", async () => {
        const { deps, manifestWrites } = makeInMemoryDeps([HANDOFF_PHRASE, HANDOFF_PHRASE, HANDOFF_PHRASE], // three dev spawns
        [NEEDS_CHANGES, NEEDS_CHANGES, READY_FOR_MERGE]);
        const result = await runDevReviewerCycle({
            targetRepoRoot: tmpRoot,
            sessionUlid: SESSION_ULID,
            ref: STORY_REF,
        ...
        // Line appears twice
        const count = result.chatLog.filter((l) => l.includes("re-spawning generalist-dev subagent (rework iteration")).length;
        expect(count).toBe(2);
    });
});
// ---------------------------------------------------------------------------
...
describe("real-fs manifest round-trip", () => {
    it("writes rework_count to disk and reads it back correctly on next read", async () => {
        const manifestPath = path.join(tmpRoot, ".crew", "state", "in-progress", `${STORY_REF}.yaml`);
        ...
        const onDisk = parseExecutionManifest(yamlParse(raw), { absPath: manifestPath });
