...
async function writeJsonl(targetRepoRoot, month, events) {
...
let targetRepoRoot;
beforeEach(async () => {
    targetRepoRoot = path.join(os.tmpdir(), `crew-compute-agreement-${crypto.randomUUID()}`);
...
describe("(a) fully-resolved window", () => {
    it("returns ratio === 0.8 for 40 agreeing + 10 disagreeing in a window of 50", async () => {
        ...
        expect(result.distribution.READY_FOR_MERGE +
            result.distribution.NEEDS_CHANGES +
            result.distribution.BLOCKED).toBe(50);
...
describe("distribution sum invariant", () => {
    it("distribution sums to windowSize across a varied verdict mix", async () => {
        ...
        if (result === null)
            return;
        const sum = result.distribution.READY_FOR_MERGE +
            result.distribution.NEEDS_CHANGES +
