...
// Main export
// ---------------------------------------------------------------------------
export async function materialisePrBranchWorktree(opts) {
    const { targetRepoRoot, sessionUlid, prNumber, role = "generalist-reviewer", pluginRootOverride, } = opts;
    ...
    // -------------------------------------------------------------------------
    const permissions = opts.permissionsOverride ??
    ...
    try {
        ...
        if (typeof parsed !== "object" ||
            ...
            throw new Error(`gh pr view --json headRefName,headRefOid returned unexpected shape: ${result.stdout}`);
        }
    ...
    // -------------------------------------------------------------------------
    const fetchResult = await runGit(["fetch", "origin", headRefName], targetRepoRoot, execaImpl);
    if (fetchResult.exitCode !== 0) {
        setupLog.push(`[materialise-pr-branch-worktree] git fetch origin ${headRefName} failed ` +
    ...
    // -------------------------------------------------------------------------
    const worktreePath = path.join(targetRepoRoot, ".crew", "state", "sessions", sessionUlid, "review-worktree");
    // -------------------------------------------------------------------------
    ...
    if (staleExists) {
        setupLog.push(`[materialise-pr-branch-worktree] stale worktree detected at ${worktreePath}; reaping.`);
        // Attempt 1: git worktree remove (handles registered worktrees cleanly).
        const reapResult = await runGit(["worktree", "remove", worktreePath, "--force"], targetRepoRoot, execaImpl);
        if (reapResult.exitCode !== 0) {
            setupLog.push(`[materialise-pr-branch-worktree] git worktree remove failed ` +
            ...
            try {
                ...
                setupLog.push(`[materialise-pr-branch-worktree] stale path removed via fs.rm.`);
            }
            catch (rmErr) {
                setupLog.push(`[materialise-pr-branch-worktree] fs.rm also failed: ${rmErr instanceof Error ? rmErr.message : String(rmErr)}. ` +
                    `Attempting worktree add anyway.`);
            }
        }
        else {
            setupLog.push(`[materialise-pr-branch-worktree] stale worktree removed successfully.`);
        }
    }
    // -------------------------------------------------------------------------
    // Step 6: git worktree add <worktreePath> <headRefOid> (use sha, not branch name).
    // -------------------------------------------------------------------------
    const addResult = await runGit(["worktree", "add", worktreePath, headRefOid], targetRepoRoot, execaImpl);
    if (addResult.exitCode !== 0) {
        throw new ReviewerPrBranchFetchError({
            prNumber,
            ghSubcommand: "pr-view",
            underlyingMessage: `git worktree add ${worktreePath} ${headRefOid} failed ` +
    ...
    async function cleanup() {
        const warnings = [];
        const removeResult = await runGit(["worktree", "remove", worktreePath, "--force"], targetRepoRoot, execaImpl);
        if (removeResult.exitCode !== 0) {
            warnings.push(`[materialise-pr-branch-worktree] cleanup: git worktree remove ${worktreePath} --force ` +
