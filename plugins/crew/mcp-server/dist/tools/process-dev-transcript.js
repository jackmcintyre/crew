...
// ---------------------------------------------------------------------------
const RECOVERABLE_ERROR_RE = /^gh-recoverable: class=(defer|retry|needs-human) subcommand=([a-z0-9-]+) exit=(\d+)/m;
// ---------------------------------------------------------------------------
...
export async function processDevTranscript(opts) {
    ...
    if (needsHumanMatch !== null) {
        ...
        chatLog.push(`needs human decision — story ${ref} paused for a human. question: ${question}`);
    ...
    if (recoverableMatch !== null) {
        const errorClass = recoverableMatch[1];
        ...
        chatLog.push(`gh recoverable error (class=${errorClass}) — story ${ref} blocked. blocked_by stamped to gh-${errorClass}. Operator action: ${actionHint}`);
        const next = errorClass === "defer"
...
// Helper
// ---------------------------------------------------------------------------
function buildActionHint(errorClass) {
    switch (errorClass) {
        case "defer":
