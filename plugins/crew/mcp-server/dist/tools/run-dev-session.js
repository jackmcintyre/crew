 ...
 */
export async function runDevSession(opts) {
    ...
    const taskSpawnWithTranscript = opts.taskSpawnWithTranscript ??
        (async (_args) => {
    ...
    const result = await runStartLoop({
        ...
        deps: {
            ...
            buildPrompt: (o) => buildPersonaSpawnPrompt({
 ...
 */
export async function readManifestFromDisk(absPath) {
 ...
 * Exported for use by `run-dev-session.test.ts` fixtures.
 */
export async function writeManifestToDisk(absPath, manifest) {
