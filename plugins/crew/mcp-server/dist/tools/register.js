...
export function registerAllTools(server) {
    ...
    server.registerTool({
        ...
        description: "Render the outstanding backlog as grouped-by-epic tables generated from live state (Story 9.5). " +
        ...
        handler: async (args) => {
            ...
            try {
                ...
                return {
                    content: [
                        { type: "text", text: renderBacklogDashboard(snapshot) },
                    ],
                };
            }
            catch (err) {
    ...
    server.registerTool({
        ...
        description: "Write a single immutable retro-proposal markdown file under " +
    ...
    server.registerTool({
        name: "acceptProposal",
        description: "The /accept-proposal <id> diff-then-confirm gate (Story 6.4, FR61, NFR10). " +
    ...
    server.registerTool({
        ...
        description: "Assemble the deterministic retro input bundle for the /crew:retro skill " +
    ...
    server.registerTool({
        ...
        description: "Parse the reviewer subagent's transcript for the verbatim locked yield phrase " +
    ...
    server.registerTool({
        ...
        description: "Classify a PR's risk tier from its diff signals (changed paths, commit messages, diff size) using the " +
    ...
    server.registerTool({
        ...
        description: "Compute the rolling reviewer-verdict vs human-merge-action agreement ratio (FR67, NFR24). " +
        ...
        handler: async (args) => {
            const parsed = {
                ...
                lastNVerdicts: args.lastNVerdicts,
            };
            try {
    ...
    server.registerTool({
        ...
        description: "Scan <targetRepoRoot>/.crew/state/in-progress/ for manifests whose claimed_by ULID " +
    ...
    server.registerTool({
        name: "reattachOrphan",
        description: "Reattach an orphaned in-progress manifest to the current session by rewriting " +
    ...
    server.registerTool({
        ...
        description: "Handle an orphaned in-progress manifest with no persisted transcript by moving it " +
