 ...
 */
export function synthesiseDecision(input) {
    ...
    if (round >= k) {
        return {
            ...
            escalation_reason: `A split panel persisted after K=${k} rounds and must not auto-pass. ` +
 ...
 */
export function adjudicationVerdictFilePath(targetRepoRoot, sessionUlid, ref) {
    return path.join(targetRepoRoot, ".crew", "state", "sessions", sessionUlid, sanitiseRefForPathSegment(ref), "adjudication-verdict.json");
}
/**
 ...
 */
export async function adjudicateQualityLead(opts) {
    ...
    const verdict = AdjudicationVerdictSchema.parse({
        ref,
