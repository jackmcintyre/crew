...
beforeEach(async () => {
    ...
    await atomicWriteFile(path.join(pluginRoot, "permissions", "orchestrator.yaml"), [
        "role: orchestrator",
        "tools_allow:",
        "  - getStatus",
        "  - recordYield",
...
// Test helpers
// ---------------------------------------------------------------------------
async function seedConfig(plugin) {
    ...
    for (let i = 0; i < opts.count; i++) {
        ...
        const event = {
            ...
            data: {
                ...
                eventual_merge_action: (agree ? "merged" : "closed-without-merge"),
            },
        };
        events.push(JSON.stringify(event));
    }
    await atomicWriteFile(path.join(telemetryDir, "2026-05.jsonl"), events.join("\n") + "\n");
}
function makeStub(opts = {}) {
    const recorder = opts.recorder ?? [];
    const stub = vi.fn().mockImplementation(async (cmd, args, callOpts) => {
    ...
    return { stub: stub, recorder };
}
function ghCalls(recorder) {
    return recorder.filter((c) => c.cmd === "gh");
}
function findCall(recorder, sub0, sub1) {
...
describe("orchestrator.yaml permission shape", () => {
    it("the production orchestrator.yaml has pr-merge in gh_allow and runAutoMergeGate in tools_allow", async () => {
        const yamlPath = path.resolve(__dirname, "..", "..", "..", "..", "permissions", "orchestrator.yaml");
...
describe("(5c) SKILL.md wiring", () => {
    it("runAutoMergeGate appears on done-ready-for-merge branch and surface lines are present", async () => {
        const skillPath = path.resolve(__dirname, "..", "..", "..", "..", "skills", "start", "SKILL.md");
