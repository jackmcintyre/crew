...
let manifestPath;
beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crew-4-12-telemetry-" + crypto.randomUUID() + "-"));
    ...
    await atomicWriteFile(path.join(tmpRoot, "team", "generalist-dev", "PERSONA.md"), DEV_PERSONA_MD);
    await atomicWriteFile(path.join(tmpRoot, "team", "generalist-reviewer", "PERSONA.md"), REVIEWER_PERSONA_MD);
});
afterEach(async () => {
...
describe("AC6: SessionQuotaExhaustedError classified from transcript", () => {
    it("processDevTranscript routes a quota-exhausted transcript to the session-quota-exhausted branch", async () => {
        ...
        expect(result.chatLog.some((l) => l.includes("session quota exhausted"))).toBe(true);
        expect(result.chatLog.some((l) => l.includes("paused"))).toBe(true);
...
describe("AC3: reviewer 8-min hard limit substitutes verdict", () => {
    it("when spawnStartedAt is more than 8 minutes ago, postReviewerComments returns reviewer-timeout and writes agent.invoke", async () => {
        ...
        await atomicWriteFile(path.join(sessionDir, "reviewer-result.json"), JSON.stringify({
            sessionUlid: SESSION_ULID,
            ref: STORY_REF,
        ...
        if (result.next !== "reviewer-timeout")
            return;
        ...
        const events = await readTelemetry();
        const invokeEvents = events.filter((e) => e.type === "agent.invoke" && e.agent === "generalist-reviewer");
        expect(invokeEvents).toHaveLength(1);
