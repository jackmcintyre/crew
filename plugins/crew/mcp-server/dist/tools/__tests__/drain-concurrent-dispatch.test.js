...
async function runDrain(opts) {
    ...
    let releaseAll = () => { };
    const allReleased = new Promise((r) => {
        releaseAll = r;
    });
    let arrived = 0;
    const onDevArrive = () => {
        arrived++;
        if (gateDev && arrived >= expectedCeiling)
            releaseAll();
    };
    ...
    const seamResult = (label) => {
        ...
        if (label.startsWith("verdict:")) {
            ...
            if (o.kind === "verdict-blocked")
                return { next: "blocked-something" };
    ...
    const agent = async (prompt, agentOpts = {}) => {
        ...
        if (label.startsWith("dev:")) {
            ...
            try {
                ...
                if (o.kind === "dev-throws")
                    throw new Error(`injected dev crash for ${ref}`);
...
function normaliseBuckets(result) {
    const byRef = (a, b) => String(a.ref ?? a).localeCompare(String(b.ref ?? b));
    return {
...
describe("drain concurrent dispatch (Story 8.22)", () => {
    ...
    const allMerge = Object.fromEntries(sixRefs.map((r) => [r, { kind: "merge" }]));
    it("AC1: runs more than one story at once, never exceeds the cap, and processes each exactly once", async () => {
        ...
        expect(normaliseBuckets(run.result).merged.map((m) => m.ref)).toEqual([...sixRefs].sort());
    ...
    it("AC2: identical result buckets and drain reason at cap 1 vs cap N (modulo intra-bucket order)", async () => {
        ...
        expect(n.merged.map((m) => m.ref)).toEqual(["s:1", "s:3"]); // merge + rework-then-merge
        expect(n.pausedForHuman.map((p) => p.ref)).toEqual(["s:2"]);
        expect(n.completed.sort()).toEqual(["s:1", "s:2", "s:3"]); // every green verdict completes
        expect(n.blocked.map((b) => b.ref).sort()).toEqual(["s:4", "s:5"]);
        // Each blocked entry preserved its reason verbatim.
        const block4 = n.blocked.find((b) => b.ref === "s:4");
        const block5 = n.blocked.find((b) => b.ref === "s:5");
    ...
    it("AC2: the same total story COUNT is processed at cap 1 and cap N (none lost or double-counted)", async () => {
        ...
        const outcomes = Object.fromEntries(refs.map((r) => [r, { kind: "merge" }]));
        const serial = await runDrain({ backlog: refs, outcomes, maxConcurrency: 1 });
        const concurrent = await runDrain({ backlog: refs, outcomes, maxConcurrency: 5, gateDev: true });
        const count = (r) => (r.completed?.length ?? 0) + (r.merged?.length ?? 0) +
        ...
        expect(concurrent.result.merged.map((m) => m.ref).sort()).toEqual([...refs].sort());
    });
    it("AC3: a worker that hard-fails is isolated — the run completes, the failure is bucketed with its reason, and every sibling reaches its bucket", async () => {
        ...
        const failed = run.result.blocked.find((b) => b.ref === "s:3");
        ...
        const merged = run.result.merged.map((m) => m.ref).sort();
        expect(merged).toEqual(["s:1", "s:4"]);
        const paused = run.result.pausedForHuman.map((p) => p.ref).sort();
        ...
        expect(run.result.blocked.map((b) => b.ref)).toEqual(["s:3"]);
    ...
    it("AC3: a hard failure under concurrency still lets the cap recycle — later stories beyond the cap are reached", async () => {
        ...
        expect(run.result.drainedReason).toBe("queue-drained");
        expect(run.result.blocked.map((b) => b.ref)).toEqual(["s:1"]);
        // All five healthy stories merged — the early crash did not stall the pool.
        expect(run.result.merged.map((m) => m.ref).sort()).toEqual([
            "s:2",
            "s:3",
            "s:4",
            "s:5",
