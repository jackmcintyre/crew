...
import { AmbiguousProposalIdError, ProposalKindNotApplicableYetError, } from "../../errors.js";
...
function teamChangeProposal(id) {
    return {
        type: "team-change",
        id,
        created_at: ISO,
...
describe("acceptProposal — locator (AC1)", () => {
    ...
    it("raises ProposalNotFoundError for an unknown id, naming files scanned", async () => {
        await expect(acceptProposal({ targetRepoRoot: tmpRoot, proposalId: ULID_MISSING })).rejects.toMatchObject({
    ...
    it("raises ProposalNotFoundError (0 scanned) when the proposals dir is absent", async () => {
        ...
        try {
            await expect(acceptProposal({ targetRepoRoot: emptyRoot, proposalId: ULID_RULE })).rejects.toMatchObject({
                name: "ProposalNotFoundError",
                filesScanned: 0,
            });
        }
    ...
    it("raises AmbiguousProposalIdError when the same id appears in two files", async () => {
        ...
        await expect(acceptProposal({ targetRepoRoot: tmpRoot, proposalId: ULID_RULE })).rejects.toBeInstanceOf(AmbiguousProposalIdError);
    });
});
// ---------------------------------------------------------------------------
...
describe("acceptProposal — preview is a no-op (AC2, AC5)", () => {
    it("returns preview + diff, writes no file, makes no commit, emits no telemetry, leaves the tree byte-identical", async () => {
        ...
        await expect(fs.access(path.join(tmpRoot, "docs", "discipline-rules.yaml"))).rejects.toThrow();
...
describe("acceptProposal — confirmed apply (AC3, AC5)", () => {
    it("applies, commits handler file + proposal file in one commit, stamps applied, emits one event", async () => {
        ...
        if (result.status === "applied") {
            expect(result.appliedSha).toBe("aabbccddeeff00112233445566778899aabbccdd");
            expect(result.idempotencyKey).toBe(ULID_RULE);
            expect(result.type).toBe("rule");
        }
        // Handler file changed on disk.
        const handlerFile = await fs.readFile(path.join(tmpRoot, "docs", "discipline-rules.yaml"), "utf8");
        ...
        expect(applied.applied_sha).toBe("aabbccddeeff00112233445566778899aabbccdd");
...
describe("acceptProposal — idempotent re-run (AC4)", () => {
    it("a second confirmed call reports already-applied, mutating nothing", async () => {
        ...
        const firstSha = first.status === "applied" ? first.appliedSha : "<none>";
...
describe("acceptProposal — unregistered kind fails closed (AC6)", () => {
    ...
    it("does not even render a preview for an unregistered kind", async () => {
        ...
        await expect(acceptProposal({ targetRepoRoot: tmpRoot, proposalId: ULID_TEAM })).rejects.toBeInstanceOf(ProposalKindNotApplicableYetError);
    });
});
