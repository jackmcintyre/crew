 ...
 */
async function seedGitRepo(repoRoot, opts = { prHeadHasArtifact: true }) {
    // Initialise
...
// ---------------------------------------------------------------------------
async function buildCrewFixture(repoRoot) {
    // .crew/config.yaml — native adapter
    await fs.mkdir(path.join(repoRoot, ".crew"), { recursive: true });
    await atomicWriteFile(path.join(repoRoot, ".crew", "config.yaml"), "adapter: native\nadapter_config: {}\n");
    ...
    await atomicWriteFile(path.join(inProgressDir, `${STORY_REF}.yaml`), [
        `ref: "${STORY_REF}"`,
        `status: in-progress`,
        `adapter: native`,
...
function makeStub(repoRoot, opts) {
    const { headRefName = "pr-head", headRefOid, ghFails = false, ghErrorStderr = "gh: not found", } = opts;
    const stub = vi.fn().mockImplementation(async (cmd, args, execOpts) => {
        if (cmd === "gh") {
            ...
            const isPrViewHeadRef = argsArr.includes("headRefName,headRefOid") ||
...
describe("AC3(d): review-worktree is torn down after session completes", () => {
    it("review-worktree directory does not exist after runReviewerSession returns", async () => {
        ...
        const worktreePath = path.join(tmpRoot, ".crew", "state", "sessions", SESSION_ULID, "review-worktree");
