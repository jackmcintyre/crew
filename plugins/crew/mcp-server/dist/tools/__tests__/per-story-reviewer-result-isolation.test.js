...
import { readReviewerResultFile, reviewerResultFilePath, sanitiseRefForPathSegment, } from "../../lib/read-reviewer-result-file.js";
 ...
 */
function makeSourceStorySpec(opts) {
    return [
        "---",
        `title: "${opts.title}"`,
...
function makeStub() {
    const stub = async (cmd, args) => {
        if (cmd === "gh") {
            const isHeadRefQuery = args.includes("headRefName,headRefOid") ||
            ...
            if (args[0] === "worktree" && args[1] === "add") {
                ...
                if (worktreePath) {
                    ...
                    for (const entry of entries) {
                        if (entry.isFile()) {
                            await fs.copyFile(path.join(tmpRoot, entry.name), path.join(worktreePath, entry.name));
                        }
                    }
                }
...
describe("AC1 — two stories in one session keep independent reviewer-result files", () => {
    it("runReviewerSession(A) then runReviewerSession(B) leaves A's persisted result intact and independently readable", async () => {
        ...
        expect(reviewerResultFilePath(tmpRoot, SESSION_ULID, REF_A)).not.toBe(reviewerResultFilePath(tmpRoot, SESSION_ULID, REF_B));
    });
