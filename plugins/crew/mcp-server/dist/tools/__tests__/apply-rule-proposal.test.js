...
import { parseRuleRegistry, DisciplineRuleSchema, } from "../../schemas/discipline-rules.js";
...
function ruleProposal(id, overrides = {}) {
    return ruleProposalObj(id, overrides);
}
// ---------------------------------------------------------------------------
...
describe("makeRuleApplyHandler — append against a seeded registry (AC2)", () => {
    it("appends a new rule with all five fields, preserving the prior rule + comment", async () => {
        ...
        const prior = data.rules.find((r) => r.target_failure_class === "handoff-grammar");
        ...
        const added = data.rules.find((r) => r.target_failure_class === "rubber-stamp");
        expect(added.id).toBe(MINTED_ULID);
        expect(added.text).toBe("Reviewer MUST verify every AC built before approving.");
    ...
    it("edits in place on a duplicate target_failure_class rather than appending", async () => {
        ...
        expect(edited.text).toBe("Dev MUST emit the handoff phrase verbatim — and only that.");
...
describe("acceptProposal production gate — rule handler registered (AC4)", () => {
    it("preview renders a diff and mutates nothing; confirm appends + commits both files + stamps + one event", async () => {
        ...
        expect(data.rules.some((r) => r.target_failure_class === "rubber-stamp")).toBe(true);
