...
async function seedRepo(root, refs) {
    // Config
    ...
    for (const ref of refs) {
        ...
        await atomicWriteFile(storyPath, `# Story ${ref}\n\nAs a dev, I want to test.\n\n## Acceptance Criteria\n\n- AC1\n`);
    }
    // Persona files
...
async function readInProgressManifest(root, ref) {
    const p = path.join(root, ".crew", "state", "in-progress", `${ref}.yaml`);
    const raw = await fs.readFile(p, "utf8");
    return parseExecutionManifest(yamlParse(raw), { absPath: p });
}
// ---------------------------------------------------------------------------
// Setup / teardown
// ---------------------------------------------------------------------------
...
describe("AC4(a) — happy handoff + READY FOR MERGE", () => {
    it("two spawns (dev then reviewer), AC1 line in chatLog, no blocked_by", async () => {
        ...
        expect(result.chatLog).toContain(`handoff received — story ${ref} — spawning generalist-reviewer subagent (clean context)`);
...
describe("AC4(c) — handoff drift block", () => {
    it("zero reviewer spawns, AC3 line in chatLog, manifest blocked_by: handoff-grammar", async () => {
        ...
        // AC3 verbatim line
        expect(result.chatLog).toContain(`handoff grammar drift — story ${ref} blocked. expected verbatim phrase: "Handoff to reviewer — story ${ref} ready for review." Edit the manifest to clear blocked_by and re-run /crew:start.`);
...
describe("AC4(d) — two-iteration rework convergence", () => {
    it("manifest rework_count: 2, AC2 line appears twice with <n>=1 and <n>=2", async () => {
        ...
        const manifest = await readInProgressManifest(tmpRoot, ref);
        expect(manifest["rework_count"]).toBe(2);
    });
});
