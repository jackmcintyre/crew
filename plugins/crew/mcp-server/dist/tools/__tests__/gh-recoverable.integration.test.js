...
beforeEach(async () => {
    ...
    await atomicWriteFile(path.join(fakePluginRoot, "permissions", "gh-error-map.yaml"), V1_ERROR_MAP);
    ...
    await atomicWriteFile(path.join(repoRoot, "team", "generalist-dev", "PERSONA.md"), FIXTURE_PERSONA_MD);
    await atomicWriteFile(path.join(repoRoot, "team", "generalist-reviewer", "PERSONA.md"), FIXTURE_REVIEWER_PERSONA_MD);
    const permissions = {
        role: "generalist-dev",
...
async function readManifestFromDisk() {
    ...
    return parseExecutionManifest(yamlParse(raw), { absPath: ctx.manifestPath });
}
// ---------------------------------------------------------------------------
...
// Helper: make gh stub
// ---------------------------------------------------------------------------
function makeGhStub(exitCode, stderr, stdout = "") {
...
describe("AC3b — defer class", () => {
    it("(i) gh() raises GhRecoverableError class=defer; (iii) manifest carries blocked_by=gh-defer; (iv) still in in-progress/", async () => {
        ...
        // (v) chat line shape
        expect(result.chatLog[0]).toBe(`gh recoverable error (class=defer) — story ${STORY_REF} blocked. blocked_by stamped to gh-defer. Operator action: wait and re-run /crew:start`);
    });
});
// ---------------------------------------------------------------------------
...
describe("AC3c — needs-human class", () => {
    it("(i) gh() raises GhRecoverableError class=needs-human; (iii) blocked_by=gh-needs-human; (iv) still in in-progress/", async () => {
        ...
        await assertManifestInInProgress();
        expect(result.chatLog[0]).toBe(`gh recoverable error (class=needs-human) — story ${STORY_REF} blocked. blocked_by stamped to gh-needs-human. Operator action: run \`gh auth login\` then re-run /crew:start`);
    });
});
// ---------------------------------------------------------------------------
// AC3d — retry class
...
describe("AC3d — retry class", () => {
    it("(i) gh() raises GhRecoverableError class=retry; (iii) blocked_by=gh-retry; (iv) still in in-progress/", async () => {
        ...
        await assertManifestInInProgress();
        expect(result.chatLog[0]).toBe(`gh recoverable error (class=retry) — story ${STORY_REF} blocked. blocked_by stamped to gh-retry. Operator action: transient network error; re-run /crew:start (v2 will auto-retry)`);
    });
});
// ---------------------------------------------------------------------------
...
entries:
  ...
  - exit_code: 4
    ...
    class: defer
`;
        await atomicWriteFile(path.join(ctx.fakePluginRoot, "permissions", "gh-error-map.yaml"), fixtureMap);
        __resetGhErrorMapCacheForTests();
        ...
        expect(authErr.class).toBe("needs-human");
        __resetGhErrorMapCacheForTests();
        ...
        expect(rateErr.class).toBe("defer");
    });
});
// ---------------------------------------------------------------------------
