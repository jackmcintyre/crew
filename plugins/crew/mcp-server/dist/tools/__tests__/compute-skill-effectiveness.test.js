...
import { computeSkillEffectiveness, SkillEffectivenessResultSchema, DEFAULT_SKILL_EFFECTIVENESS_WINDOW, } from "../compute-skill-effectiveness.js";
...
function assertValid(event) {
    ...
    if (!result.success) {
        throw new Error(`Fixture event not TelemetryEventSchema-valid: ${JSON.stringify(result.error.issues)}`);
    }
    return event;
}
/**
 ...
 */
function seams(files) {
    const names = Object.keys(files).sort();
    return {
        readTelemetryDirImpl: async () => names,
        readFileImpl: async (filePath) => {
            const name = filePath.split("/").pop();
            const lines = (files[name] ?? []).map((e) => typeof e === "string" ? e : JSON.stringify(e));
            return lines.join("\n") + "\n";
        },
    };
}
...
describe("computeSkillEffectiveness — AC3 edges", () => {
    ...
    it("throws SkillEffectivenessWindowInvalidError on an invalid window", async () => {
        for (const bad of [0, -1, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
            await expect(computeSkillEffectiveness({
                targetRepoRoot: ROOT,
                window: bad,
                ...seams({ "2026-05.jsonl": [] }),
