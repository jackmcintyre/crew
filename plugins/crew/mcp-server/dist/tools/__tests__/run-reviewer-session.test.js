 ...
 */
async function createWorktreeFromTmpRoot(worktreePath, tmpRoot) {
    ...
    for (const entry of entries) {
        if (entry.isFile()) {
            await fsP.copyFile(path.join(tmpRoot, entry.name), path.join(worktreePath, entry.name));
        }
    }
}
function makeDiscriminatingStub(opts = {}) {
    const stub = vi.fn().mockImplementation(async (cmd, args, _cmdOpts) => {
        if (cmd === "gh") {
            const argsArr = args;
...
async function buildFixture(tmpRoot) {
    ...
    await atomicWriteFile(path.join(tmpRoot, "package.json"), JSON.stringify({ name: "fixture-4-6", version: "0.0.0", private: true }, null, 2));
