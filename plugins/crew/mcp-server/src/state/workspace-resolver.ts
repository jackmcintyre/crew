 ...
 */
export async function resolveWorkspace(opts: ResolveWorkspaceOptions): Promise<Workspace> {
  ...
  if (!(await fileExists(configPath))) {
    ...
    const detectResults = await Promise.all(
      adapters.map((a) => a.detect(targetRepoRoot)),
    );
  ...
  if (!adapterParsed.success) {
    ...
    const subPath =
      issue.path.length === 0 ? "adapter_config" : `adapter_config.${issue.path.join(".")}`;
    throw new InvalidWorkspaceConfigError({
      configPath,
      yamlPath: subPath,
      zodMessage: issue.message,
