 ...
 */
export async function createSmokeScratchRepo(
  opts: CreateSmokeScratchRepoOptions,
): Promise<CreateSmokeScratchRepoResult> {
  ...
  const scratchRoot = await fs.mkdtemp(
    path.join(parentDir ?? os.tmpdir(), `crew-smoke-${label}-`),
  );
