...
function refFor(ulid: string): string {
  return `native:${ulid}`;
}

/** Build a minimal native story body. */
function makeStoryBody(suffix: string): string {
  return [
...
function makeManifestYaml(opts: {
  ...
  const manifest = {
    ...
    narrative:
      "As a user, I want resilience so that one bad file doesn't break everything.",
    withdrawn: false,
  };
  ...
  it("skips a single malformed-yaml manifest with reason 'unreadable-manifest' and scans the other three clean", async () => {
    ...
    for (let i = 0; i < STORY_ULIDS.length; i++) {
      ...
      if (i < 3) {
        ...
        await atomicWriteFile(
          toDoManifestAbsPath,
          makeManifestYaml({ ref, sourceHash, sourcePath: storyRelPath }),
        );
      } else {
