...
import {
  ...
  synthesiseDecision,
  adjudicationVerdictFilePath,
  DEFAULT_ADJUDICATION_K,
} from "../quality-lead-adjudicate.js";
...
async function seedTodo(opts: { ready?: boolean } = {}): Promise<string> {
...
async function readReadyFlag(absPath: string): Promise<boolean | undefined> {
  const raw = await fs.readFile(absPath, "utf8");
  return (yamlParse(raw) as { ready?: boolean }).ready;
}
...
describe("AC2: the synthesis rule decides ready / rework / escalate", () => {
  ...
  it("synthesiseDecision is a pure function over the panel (default K = 2)", () => {
    ...
    expect(
      synthesiseDecision({ panel: makePanel({ domain: "ungrounded claim" }), round: 1, k: 2 }).decision,
    ).toBe("rework");
    expect(
      synthesiseDecision({ panel: makePanel({ domain: "still split" }), round: 2, k: 2 }).decision,
    ).toBe("escalate");
  });
});
...
describe("AC3: ready blesses via the Story 9.1 brake; a non-ready decision leaves the draft not-ready", () => {
  ...
  it("blesses ONLY through the brake — the brake seam is exercised, no direct manifest write", async () => {
    const absPath = await seedTodo({ ready: false });
    let brakeCalledWith: { ref: string; ready: boolean } | null = null;
...
describe("AC5: the emitted verdict validates against the schema and is persisted", () => {
  ...
  it("emits exactly one quality.adjudicated telemetry event, even on a ready decision", async () => {
    ...
    const events = lines.map((l) => JSON.parse(l) as { type: string; data?: { decision?: string; escalated?: boolean } });
  ...
  it("the schema rejects an escalate verdict with no escalation_reason (the close-call-auto-pass guard)", () => {
    expect(() =>
      AdjudicationVerdictSchema.parse({
        ref: REF,
        decision: "escalate",
        rationale: "split",
        round: 2,
