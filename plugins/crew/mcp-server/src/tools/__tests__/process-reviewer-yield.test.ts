...
async function seedManifest(manifestPath: string, ref: string): Promise<void> {
...
async function readOnDiskManifest(manifestPath: string): Promise<ExecutionManifest> {
  const raw = await fs.readFile(manifestPath, "utf8");
  return parseExecutionManifest(yamlParse(raw), { absPath: manifestPath });
}

async function readTelemetryJsonl(targetRepoRoot: string): Promise<unknown[]> {
  const now = new Date();
...
describe("processReviewerYield — success branch (sub-case a)", () => {
  it("routes yield to security-specialist and emits yield.handoff telemetry", async () => {
    ...
    if (result.next !== "spawn-specialist-reviewer") throw new Error("Type narrowing");
