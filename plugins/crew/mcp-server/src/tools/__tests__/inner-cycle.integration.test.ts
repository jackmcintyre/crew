...
describe("AC4(g): tool count and required tools present", () => {
  it("registered tool list has exactly 27 entries and contains the required tools but NOT runDevSession", async () => {
    ...
    try {
      ...
      expect(toolNames.length).toBe(30); // Story 4.12 added recordAgentInvoke (26), recordPrCloseAction (27); Story 4.11 added processReviewerYield (28); Story 4.9b added classifyRiskTier (29); Story 4.10 added computeAgreement (30)
    } finally {
      await client.close();
      await server.close();
    }
...
describe("AC4 (4.3c): green-path two-story drain via processReviewerTranscript internal seam", () => {
  ...
  it("drives two stories through claim → dev → reviewer-ready (internal completeStory), then queue-drained", async () => {
    ...
    await expect(
      fs.stat(path.join(root, ".crew", "state", "to-do", `${refA}.yaml`)),
    ).rejects.toThrow();
    ...
    const doneManifestBRaw = await fs.readFile(
      path.join(root, ".crew", "state", "done", `${refB}.yaml`),
      "utf8",
    );
