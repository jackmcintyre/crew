...
describe("AC4(g): tool count and required tools present", () => {
  it("registered tool list has exactly 27 entries and contains the required tools but NOT runDevSession", async () => {
    ...
    try {
      ...
      expect(toolNames.length).toBe(29); // Story 4.12 added recordAgentInvoke (26), recordPrCloseAction (27); Story 4.11 added processReviewerYield (28); Story 4.9b added classifyRiskTier (29)
    } finally {
      await client.close();
      await server.close();
    }
