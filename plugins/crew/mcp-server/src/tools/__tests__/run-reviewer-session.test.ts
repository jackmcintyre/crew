...
describe("AC4(g): failing vitest test → acResults[2].status === 'fail'", () => {
  it("rewrites fixture test to fail; AC2 status === fail, exitCode !== 0", { timeout: 60_000 }, async () => {
    ...
    await atomicWriteFile(
      path.join(tmpRoot, "__tests__", "fixture.test.ts"),
      FIXTURE_VITEST_FAILING_TEST,
    );
    ...
    const ghStub = vi.fn().mockImplementation(
    ...
    const result = await runReviewerSession({
      ...
      ref: STORY_REF,
      prNumber: PR_NUMBER,
      execaImpl: ghStub as unknown as typeof import("execa").execa,
    });
