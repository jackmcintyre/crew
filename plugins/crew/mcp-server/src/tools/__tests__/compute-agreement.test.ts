...
async function makeHarness(): Promise<ServerHarness> {
  const server = createServer();
  registerAllTools(server);
  const [clientTransport, serverTransport] =
    InMemoryTransport.createLinkedPair();
  const client = new Client(
    { name: "compute-agreement-test", version: "0.0.0" },
    { capabilities: {} },
  );
  await Promise.all([
...
beforeEach(async () => {
  tmpRoot = path.join(
    os.tmpdir(),
    `crew-compute-agreement-tool-${crypto.randomUUID()}`,
  );
...
async function seedJsonl(events: Array<Record<string, unknown>>) {
  const dir = path.join(tmpRoot, ".crew", "telemetry");
  await fs.mkdir(dir, { recursive: true });
  await atomicWriteFile(
    path.join(dir, "2026-05.jsonl"),
...
describe("computeAgreement MCP tool boundary", () => {
  ...
  it("rejects `lastNVerdicts: 0`", async () => {
    await expect(
      call({ targetRepoRoot: tmpRoot, lastNVerdicts: 0 }),
    ).rejects.toThrow();
  });

  it("rejects `lastNVerdicts: -5`", async () => {
    await expect(
      call({ targetRepoRoot: tmpRoot, lastNVerdicts: -5 }),
    ).rejects.toThrow();
  });

  it('rejects `lastNVerdicts: "fifty"` (wrong type)', async () => {
    await expect(
      call({ targetRepoRoot: tmpRoot, lastNVerdicts: "fifty" }),
    ).rejects.toThrow();
  });
