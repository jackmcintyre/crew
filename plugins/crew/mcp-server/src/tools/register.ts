...
export function registerAllTools(server: AiEngineeringTeamServer): void {
  ...
  server.registerTool({
    ...
    description:
      "Compute the rolling reviewer-vs-eventual-action agreement ratio over the " +
  ...
  server.registerTool({
    ...
    description:
      "Decide auto-merge vs needs-human pause for the just-completed reviewer run (FR40, FR41, FR42).",
    inputSchema: {
      type: "object",
      properties: {
        targetRepoRoot: { type: "string" },
  ...
  server.registerTool({
    ...
    description:
      "Return the list of in-progress dev claims that have exceeded the per-story budget (default 30 min, NFR3).",
    inputSchema: {
      type: "object",
      properties: {
        targetRepoRoot: { type: "string" },
  ...
  server.registerTool({
    ...
    description:
      "Stamp `blocked_by: reviewer-timeout` on the in-progress manifest after the reviewer subagent exceeded the 8-minute hard limit (NFR2).",
    inputSchema: {
      type: "object",
      properties: {
        targetRepoRoot: { type: "string" },
  ...
  server.registerTool({
    ...
    description:
      "Create a disposable smoke-harness scratch repo at " +
