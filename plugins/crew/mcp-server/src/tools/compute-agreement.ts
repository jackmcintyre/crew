...
import type {
  ...
  ReviewerVerdictMergeActionEvent,
...
export async function computeAgreement(
  opts: ComputeAgreementOptions,
): Promise<AgreementMetricResult | null> {
  const {
    targetRepoRoot,
    lastNVerdicts: rawWindow,
  ...
  const mergeActions: ReviewerVerdictMergeActionEvent[] = [];
  ...
  type ResolvedPair = {
    ...
    mergeAction: "merged" | "closed-unmerged";
    ts: string;
    session_id: string;
  };
  ...
  const distribution: AgreementMetricResult["distribution"] = {
    "READY FOR MERGE": 0,
    "NEEDS CHANGES": 0,
    BLOCKED: 0,
  };
