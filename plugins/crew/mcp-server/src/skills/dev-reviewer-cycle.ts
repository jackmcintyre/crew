...
export type RunDevReviewerCycleFinalState =
  | "ready-for-merge"
  | "needs-changes-resolved"
  | "blocked-handoff-grammar"
  | "blocked-reviewer-grammar"
  | "blocked-reviewer-verdict";
 ...
 */
export async function runDevReviewerCycle(
  opts: RunDevReviewerCycleOptions,
): Promise<RunDevReviewerCycleResult> {
  ...
  while (true) {
    ...
    const devInitialContext: Record<string, unknown> = {
      ref,
      title,
      sessionUlid,
      targetRepoRoot,
    ...
    if (!handoffResult.ok) {
      ...
      const blockedManifest: ExecutionManifest = {
        ...currentManifest,
    ...
    const reviewerInitialContext: Record<string, unknown> = {
      ref,
      title,
    ...
    const { transcript: reviewerTranscript } =
      await deps.taskSpawnWithTranscript({
    ...
    const reworkManifest: ExecutionManifest = {
      ...currentManifest,
    ...
    chatLog.push(
      `reviewer verdict: NEEDS CHANGES — re-spawning generalist-dev subagent (rework iteration ${localReworkCount})`,
    );
