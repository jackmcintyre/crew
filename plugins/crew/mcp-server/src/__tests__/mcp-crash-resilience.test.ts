...
describe("AC3a — uncaughtException handler installed and server survives healthy operation", () => {
  it(
    "server continues to respond to tools/list after steady-state window (handler present, no-exit contract)",
    async () => {
...
describe("AC3b — unhandledRejection handler installed, server does not crash", () => {
  it(
    "server remains alive and responsive after init (unhandledRejection default-exit suppressed)",
    async () => {
      const logPath = path.join(tmpDir, "lifecycle.log");
      child = spawnServer(logPath);
...
describe("AC3c — SIGTERM logs signal event and exits with code 143", () => {
  it(
    "SIGTERM: log contains signal{name:SIGTERM} and process exits with code 143",
    async () => {
      const logPath = path.join(tmpDir, "lifecycle.log");
      child = spawnServer(logPath);
...
describe("AC3c — SIGINT logs signal event and exits with code 130", () => {
  it(
    "SIGINT: log contains signal{name:SIGINT} and process exits with code 130",
    async () => {
      const logPath = path.join(tmpDir, "lifecycle.log");
      child = spawnServer(logPath);
...
describe("AC3c — SIGHUP logs signal event and exits with code 129", () => {
  it(
    "SIGHUP: log contains signal{name:SIGHUP} and process exits with code 129",
    async () => {
      const logPath = path.join(tmpDir, "lifecycle.log");
      child = spawnServer(logPath);
...
describe("AC3d — main().catch preserved in dist/index.js", () => {
  it(
    "dist/index.js contains main().catch with process.exit(1)",
    () => {
      ...
      const hasCatchPattern =
        /main\(\)\s*\.catch/.test(distSrc) &&
