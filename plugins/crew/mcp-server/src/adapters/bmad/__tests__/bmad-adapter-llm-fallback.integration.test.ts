...
const callLog: Array<{ model: string; userText: string }> = [];

vi.mock("../../../lib/anthropic-client.js", () => {
  return {
    hasAnthropicKey: () => true,
    getAnthropicClient: () => ({
      async createMessage(opts: { model: string; userText: string }) {
        ...
        if (opts.userText.includes("3-2-drifted-em-dash-acs.md")) {
          return {
            text: JSON.stringify({
              ...
              narrative:
                "As a plugin operator, I want a story whose AC headings drift, so that the LLM fallback can recover it.",
              acceptance_criteria: [
...
describe("BMad adapter LLM fallback (Story 3.9)", () => {
  ...
  it("clean story parses via regex; LLM mock is NOT invoked", async () => {
    ...
    const cleanCalls = callLog.filter((c) =>
      c.userText.includes("3-1-clean-story.md"),
    );
  ...
  it("drifted story is recovered by the LLM fallback; manifest in to-do/", async () => {
    ...
    const driftedCalls = callLog.filter((c) =>
      c.userText.includes("3-2-drifted-em-dash-acs.md"),
    );
  ...
  it("genuinely-broken file fails both paths; routed to blocked/ with blocked_by: unparseable", async () => {
    ...
    const warning = result.warnings.find((w) =>
      w.path.includes("3-3-genuinely-broken.md"),
    );
  ...
  it("cache: a successfully-extracted drifted story is cached and triggers zero new calls on re-scan", async () => {
    ...
    const driftedCallsFirst = callLog.filter((c) =>
      c.userText.includes("3-2-drifted-em-dash-acs.md"),
    ).length;
    ...
    await scanSources({ targetRepoRoot: root });
    const driftedCallsTotal = callLog.filter((c) =>
      c.userText.includes("3-2-drifted-em-dash-acs.md"),
    ).length;
  ...
  it("skip-done filter: a Status: done file is not parsed and not extracted", async () => {
    ...
    const tmpStories = await fs.mkdtemp(
      path.join(os.tmpdir(), "crew-test-3-9-done-"),
    );
    try {
      await atomicWriteFile(
        path.join(tmpStories, "9-1-done-story.md"),
        [
      ...
      expect(
        newCalls.filter((c) => c.userText.includes("9-1-done-story.md")),
      ).toHaveLength(0);
    } finally {
