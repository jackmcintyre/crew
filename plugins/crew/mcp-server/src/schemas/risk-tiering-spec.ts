...
export const DiffSizeThresholdsSchema = z
  ...
  .strict()
  .refine(
    (v) => v.min_lines_changed !== undefined || v.max_lines_changed !== undefined,
    {
...
export const RuleSchema = z
  ...
  .refine(
    (rule) =>
      rule.path_patterns !== undefined ||
      rule.change_types !== undefined ||
      rule.diff_size_thresholds !== undefined ||
      rule.additive_only !== undefined,
    { message: "rule declares no signal fields" },
  );
...
export const RiskTieringSpecSchema = z
  .object({
    ...
    fallback_tier: z.literal("medium", {
      message:
        "fallback_tier must be 'medium' (v1 invariant — see Architecture § Risk-tier classification, Fallback)",
    }),
    tiers: z
      .object({
      ...
      .refine(
        (tiers) =>
          (tiers.low?.length ?? 0) +
            ...
            (tiers.high?.length ?? 0) >
          0,
        { message: "no rules declared in any tier" },
      ),
  })
  .strict();
