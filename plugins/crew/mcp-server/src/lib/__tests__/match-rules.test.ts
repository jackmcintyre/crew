...
function makePathRule(patterns: string[]): Rule {
  return { id: "test-path-rule", path_patterns: patterns };
}

function makeChangeTypeRule(types: Array<"revert" | "migration" | "schema" | "dep-bump">): Rule {
  return { id: "test-ct-rule", change_types: types };
}

function makeSizeRule(opts: { min?: number; max?: number }): Rule {
  return {
    id: "test-size-rule",
...
describe("matchRule — additive_only", () => {
  const rule: Rule = { id: "low.additive-only", additive_only: true };
  ...
  it("AND-combines with diff_size_thresholds — additive AND within size matches", () => {
    ...
    expect(
      matchRule(combo, { changedPaths: ["a.ts"], detectedChangeTypes: [], diffSize: 200, additiveOnly: true }).matched,
    ).toBe(true);
    // additive but over the size cap → no match
    expect(
      matchRule(combo, { changedPaths: ["a.ts"], detectedChangeTypes: [], diffSize: 400, additiveOnly: true }).matched,
    ).toBe(false);
  });
});
