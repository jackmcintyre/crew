...
const LOCK_STDERR =
  "fatal: could not lock config file /tmp/work/.git/config: File exists";
...
function isWorktreeAdd(cmd: string, args: readonly string[]): boolean {
 ...
 */
function makeFlakyAddExeca(opts: { failAdds: number; stderr: string; exitCode?: number }) {
  let addAttempts = 0;
  const impl = async (
    cmd: string,
    ...
    const r = await realExeca(cmd, args as string[], { ...options, reject: false });
    return {
...
describe("materialiseDevStoryWorktree — concurrent worktree-add lock retry", () => {
  it("retries a transient lock-contention failure and succeeds", async () => {
    ...
    const result = await materialiseDevStoryWorktree({
      ...
      ref: REF,
      base: "main",
      execaImpl: flaky.impl as unknown as Parameters<typeof materialiseDevStoryWorktree>[0]["execaImpl"],
      sleepImpl: noopSleep,
    });
