...
import {
  findStuckDevClaims,
  DEV_BUDGET_MS_DEFAULT,
} from "../find-stuck-dev-claims.js";
...
beforeEach(async () => {
  tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "crew-find-stuck-" + crypto.randomUUID() + "-"),
  );
});
...
async function seedManifest(opts: SeedOpts): Promise<void> {
...
describe("findStuckDevClaims — 30-min dev budget surfaces in next poll", () => {
  ...
  it("strict greater-than: a claim exactly at budget is NOT stuck", async () => {
    ...
    const claimedAt = new Date(
      now.getTime() - DEV_BUDGET_MS_DEFAULT,
    ).toISOString();
    await seedManifest({ ref: "native-A", claimedAt });
