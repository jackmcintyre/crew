...
beforeEach(async () => {
  tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "crew-agent-invoke-" + crypto.randomUUID() + "-"),
  );
});
...
describe("SessionQuotaExhaustedError classified from transcript", () => {
  it("detectSessionQuotaExhausted matches the canonical 'You've hit your session limit' string", () => {
    expect(
      detectSessionQuotaExhausted("Some output\nYou've hit your session limit\nmore"),
    ).toBe(true);
  });

  it("matches account-limit variant", () => {
    expect(detectSessionQuotaExhausted("You've hit your account limit")).toBe(
      true,
    );
  });

  it("matches without curly apostrophe", () => {
    expect(detectSessionQuotaExhausted("You have hit your session limit")).toBe(
      true,
    );
  });
