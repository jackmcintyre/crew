 ...
 */
export function composeVerdictLine(result: ReviewerResultFileShape): string {
  ...
  if (recommendedVerdict === "NEEDS CHANGES") {
    const failCount = Object.values(acResults).filter(
      (r) =>
        (r.applicability === "runnable-artifact-check" ||
          r.applicability === "runnable-vitest") &&
        r.status === "fail",
    ).length;
    const manualCount = Object.values(acResults).filter(
      (r) => r.applicability === "manual-check-required",
    ).length;
...
export function composeSummaryBody(result: ReviewerResultFileShape): string {
  ...
  const manualAcs = acEntries.filter(
    ({ ac }) => ac.applicability === "manual-check-required",
  );
...
function formatAcLine(index: number, ac: AcResult): string {
