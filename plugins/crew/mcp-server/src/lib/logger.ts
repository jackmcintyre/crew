...
function toIsoMillisUtc(d: Date): string {
  // Date#toISOString() always produces ms-precise UTC ending in `Z`.
  return d.toISOString();
}

function monthBucket(ts: string): string {
 ...
 */
export async function logTelemetryEvent(opts: LogTelemetryEventOpts): Promise<void> {
  ...
  const attemptedType = String(
    (stamped as { type?: unknown }).type ?? "<missing>",
  );
