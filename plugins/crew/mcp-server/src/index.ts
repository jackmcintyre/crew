 ...
 */
const _keepAliveHandle: ReturnType<typeof setInterval> = setInterval(
  () => {
    /* intentional no-op: keep the event loop alive */
  },
  1 << 30, // ~12 days; effectively never fires
);
 ...
 */
function swallowStdinEnd(): void {
  if (process.env["CREW_MCP_DIAG"]) {
    // eslint-disable-next-line no-console
    process.stderr.write(
      JSON.stringify({ event: "stdin.end.swallowed", pid: process.pid }) + "\n",
    );
  }
}

function swallowStdinClose(): void {
  if (process.env["CREW_MCP_DIAG"]) {
    // eslint-disable-next-line no-console
    process.stderr.write(
      JSON.stringify({ event: "stdin.close.swallowed", pid: process.pid }) + "\n",
    );
  }
}
