...
Scope: a one-off, LLM-assisted, reviewed migration — `parseBmadStory` → `SourceStory` → LLM enriches to the §3 shape (infer per-AC verification, tasks, cited_sources from the prose) → discipline gate → `writeNativeStory`. One-way, never a live sync (LLM transforms are lossy — fine for seeding, fatal as a dependency). Runs over the live `bmad:*` backlog once to seed `.crew/native-stories/`; stories that can't be enriched to clear Tier-0 surface for human fix-up, not silent drop. *Observable spine: running the ingest over a BMad story produces a native story that clears Tier-0, OR surfaces it in a fix-up list with the failed checks named — never silently dropped.* (Slice 5. Depends on 10.1–10.3.)
