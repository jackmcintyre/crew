...
**Then** `runAutoMergeGate` returns `{ next: "paused-residual-medium-or-higher", prNumber, residuals: { medium: number, high: number } }` and emits chat line `PR #${prNumber} paused — ${count} unresolved medium/high finding(s)`. _(retro carry-forward #6 — see Retro Amendments section below for the why)_

<!-- Substrate: the chat line is operator-facing but the AC's verifiable surface is the tool return shape. -->
